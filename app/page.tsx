...
export default function Home() {
  ...
  const handleNext = async (key: keyof UserInput, value: string) => {
    ...
    // Trigger haptic tick if enabled
  ...
  return (
    <main className="min-h-screen bg-premium-gradient text-foreground flex flex-col relative selection:bg-brand-neon selection:text-black overflow-x-hidden">
      ...
      {/* Brand Header */}
      <header className="w-full p-6 md:p-8 z-50 relative flex justify-between items-center">
        <h1 className="text-3xl md:text-4xl font-heading font-black uppercase tracking-tighter italic transform -skew-x-12 origin-left">
          Lumina <span className="text-brand-neon text-sm align-top ml-1 not-italic skew-x-0 inline-block bg-white/10 px-1 rounded">V2</span>
        </h1>
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col items-center justify-center p-4 w-full max-w-7xl mx-auto relative z-10 pb-20">
        <AnimatePresence mode="wait">
          {step === "gender" && (
            <motion.div
              ...
              className="text-center space-y-8 w-full max-w-md bg-black/40 backdrop-blur-xl p-8 border border-white/10 rounded-3xl"
            >
              <h2 className="text-3xl md:text-5xl font-heading font-black uppercase tracking-tight">Identify</h2>
          ...
          {step === "age" && (
            <motion.div
              ...
              className="text-center space-y-8 w-full max-w-md bg-black/40 backdrop-blur-xl p-8 border border-white/10 rounded-3xl"
            >
              <h2 className="text-3xl md:text-5xl font-heading font-black uppercase tracking-tight">Chronology</h2>
              <div className="flex flex-col gap-6">
                <input
                  type="number"
                  placeholder="00"
                  className="bg-black/50 border-2 border-white/10 text-center text-6xl md:text-7xl py-6 focus:outline-none focus:border-brand-neon transition-all duration-300 text-brand-neon font-heading font-black placeholder:text-white/5 w-full rounded-2xl"
          ...
          {step === "goal" && (
            <motion.div
              ...
              className="text-center space-y-8 w-full max-w-md bg-black/40 backdrop-blur-xl p-8 border border-white/10 rounded-3xl"
            >
              <h2 className="text-3xl md:text-5xl font-heading font-black uppercase tracking-tight">Objective</h2>
              <div className="grid grid-cols-1 gap-4">
          ...
          {step === "occasion" && (
            <motion.div
              ...
              className="text-center space-y-8 w-full max-w-md bg-black/40 backdrop-blur-xl p-8 border border-white/10 rounded-3xl"
            >
              <h2 className="text-3xl md:text-5xl font-heading font-black uppercase tracking-tight">Context</h2>
              <div className="grid grid-cols-1 gap-4">
          ...
          {step === "result" && isGenerating && (
            ...
            >
              ...
              <h2 className="text-2xl font-mono text-brand-neon animate-pulse uppercase tracking-widest">
